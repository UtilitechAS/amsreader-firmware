...
//  import ConsentComponent from "./lib/ConsentComponent.svelte"; //EHorvat disabled consent stuff
  ...
  sysinfoStore.subscribe(update => {
    ...
    if(sysinfo.vndcfg === false) {
      ...
      navigate("/setup");
  //  } else if(sysinfo.fwconsent === 0) {      //EHorvat disabled consent stuff
  //    navigate("/consent");                   //EHorvat disabled consent stuff
    }
  });
  getSysinfo();
  let data = {};
