<script>
    import { pricesStore, dayPlotStore, monthPlotStore, temperaturesStore, hourPlotStore} from './DataStores.js';  //EHorvat added: , hourPlotStore
    ...
    hourPlotStore.subscribe(update => {     //EHorvat hour plot added line
        hourPlot = update;                  //EHorvat hour plot added line
    });                                     //EHorvat hour plot added line
...
<div class="grid 2xl:grid-cols-6 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2">
    ...
    {/if}
<!-- EHorvat 1 hour plot start   hourPlot is visible when DayPlot is visible -->
