...
let networks = { c: -1, n: [] };
const networkOrder = new Map();
let networkOrderCounter = 0;
let networksTimer = null;
let networksActive = false;
let networksSet = null;
let networksPromise = null;

async function runNetworksScan() {
    try {
        const response = await fetchWithTimeout("wifiscan.json", { timeout: 30000 });
        const res = await response.json();
        const latestPerSsid = new Map();

        if(Array.isArray(res?.n)) {
            for (let i = 0; i < res.n.length; i++) {
                const net = res.n[i];
                if(!net || typeof net.s !== 'string' || !net.s.length) {
                    continue;
                }
                let order = networkOrder.get(net.s);
                if(order === undefined) {
                    order = networkOrderCounter++;
                    networkOrder.set(net.s, order);
                }
                latestPerSsid.set(net.s, { ...net, _order: order });
            }
        }

        const list = Array.from(latestPerSsid.values())
        ...
        if(networksSet) {
            networksSet(networks);
        }
    } catch (err) {
        if(networks.c < 0 && networksSet) {
            networksSet({ c: -1, n: [] });
        }
    } finally {
...
function fetchNetworks() {
    if(!networksPromise) {
        networksPromise = runNetworksScan();
    }
    return networksPromise;
}

function requestNetworksRefresh(delay = 0) {
    if(!networksActive) {
        return;
    }
    if(networksTimer) {
        clearTimeout(networksTimer);
    }
    networksTimer = setTimeout(() => {
        networksTimer = null;
        fetchNetworks();
    }, Math.max(0, delay));
}
    ...
    async function getData() {
        fetchWithTimeout("data.json")
            ...
            .then((data) => {
                ...
                if(networksActive && !pendingNetworkRefresh) {
                    pendingNetworkRefresh = true;
                    Promise.resolve().then(() => {
                        pendingNetworkRefresh = false;
                        requestNetworksRefresh(0);
                    });
                }
            })
            .catch((err) => {
