...
let sysinfo = {
    ...
    vndcfg: null,
    usrcfg: null,
//    fwconsent: null,//EHorvat disabled consent stuff
    booting: false,
    upgrading: false,
    ui: {},
    security: 0,
        ...
        fetchWithTimeout("/data.json")
            ...
            .then((data) => {
                ...
                if(sysinfo.upgrading) {
                    ...
                    if(hourPlotTimeout) clearTimeout(hourPlotTimeout);  //EHorvat new hour plot
                    hourPlotTimeout = setTimeout(getHourPlot, 2500);    //EHorvat new hour plot
                }
                let to = 5000;
...
// EHorvat new hour plot start
let hourPlot = {};
let hourPlotTimeout;
export async function getHourPlot() {
    if(hourPlotTimeout) {
        clearTimeout(hourPlotTimeout);
        hourPlotTimeout = 0;
    }
    const response = await fetchWithTimeout("/hourplot.json");
    hourPlot = (await response.json())
    hourPlotStore.set(hourPlot);

    let date = new Date();
    hourPlotTimeout = setTimeout(getHourPlot, 20030)  //Update data every 20 sec
}

export const hourPlotStore = writable(hourPlot, (set) => { 
    getHourPlot();
    return function stop() {}
});
