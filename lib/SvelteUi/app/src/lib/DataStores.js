...
async function runNetworksScan() {
    try {
        ...
        const strongestPerSsid = new Map();
        ...
        if(Array.isArray(res?.n)) {
            for (let i = 0; i < res.n.length; i++) {
                ...
                const current = strongestPerSsid.get(net.s);
                if(!current || Number(net.r) > Number(current.r)) {
                    strongestPerSsid.set(net.s, net);
                }
            }
        }

        const list = Array.from(strongestPerSsid.values())
