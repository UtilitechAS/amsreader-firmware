<script>
  ...
  //import { upgrade, getNextVersion, upgradeWarningText } from './UpgradeHelper';   //EHorvat disabled consent stuff  
  ...
  // import DownloadIcon from "./DownloadIcon.svelte";    EHorvat disabled consent stuff  
...
/*  EHorvat disabled consent stuff  
  ...
  sysinfoStore.subscribe(update => {
    sysinfo = update; /*  EHorvat disabled consent stuff  
...
<nav class="bg-violet-600 p-1 rounded-md mx-2">
    ...
    <div class="flex-auto p-2 flex flex-row-reverse flex-wrap">
        <div class="flex-none">
          <a class="float-right" href='https://github.com/ehorvat1/AmsToMqttBridge-EHo/tree/master' target='_blank' rel="noreferrer" aria-label="GitHub"><img class="gh-logo" src={GitHubLogo} alt="GitHub repo"/></a>
        </div>
        ...
        </div>        <!--  EHorvat disabled consent stuff
