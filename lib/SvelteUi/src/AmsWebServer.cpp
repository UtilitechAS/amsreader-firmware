...
#include "html/hourplot_json.h" //EHorvat new hour plot (dont forget to modify conf_ui.json...add     "h": %d, after line 9, 
								// and same also for sysinfo.json add after line 34)
								// Add also this       "/hourplot.json": "http://192.168.233.235",    to vite.config.js after line 21
...
void AmsWebServer::setup(AmsConfiguration* config, GpioConfig* gpioConfig, MeterConfig* meterConfig, AmsData* meterState, AmsDataStorage* ds, EnergyAccounting* ea) {
	...
	// server.on(F("/consent"), HTTP_GET, std::bind(&AmsWebServer::indexHtml, this));  //EHorvat disabled consent stuff
	...
	server.on(F("/hourplot.json"), HTTP_GET, std::bind(&AmsWebServer::hourplotJson, this));  //EHorvat new hour plot
...
// 	server.on(F("/upgrade"), HTTP_POST, std::bind(&AmsWebServer::upgrade, this));//EHorvat disabled consent stuff
...
//	bool access = WiFi.getMode() == WIFI_AP || webConfig.security < level;   	//EHorvat deleted this line ....see next line
	bool access = webConfig.security < level;  									//EHorvat new ... enforce security also when in AP mode
...
//		sys.dataCollectionConsent,					//EHorvat disabled consent stuff  NOTE: also delete line 10 ("fwconsent": %d,) from sysinfo.json...chrashes if not
		hostname.c_str(),
		...
		ui.showPeaks,
		ui.showPricePlot,
		ui.showHourPlot,					//EHorvat new hour plot
		ui.showDayPlot,
		ui.showMonthPlot,
...
void AmsWebServer::dayplotJson() {
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Serving /dayplot.json over http...\n")); //EHorvat changed to Info level
...
void AmsWebServer::monthplotJson() {
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Serving /monthplot.json over http...\n")); //EHorvat changed to Info level
...
// minimize JSON size and use signed int (value = imp - exp)
void AmsWebServer::hourplotJson() {
	if(debugger->isActive(RemoteDebug::INFO)) debugger->printf_P(PSTR("Serving /hourplot.json over http...\n"));  //EHorvat changed to Info level
	...
	if(ds == NULL) {
		...
		snprintf_P(buf, BufferSize, HOURPLOT_JSON,
			ds->getHourWatt(0),
			ds->getHourWatt(1),
			ds->getHourWatt(2),
			ds->getHourWatt(3),
...
//		sys.dataCollectionConsent = 0;//EHorvat disabled consent stuff
		config->setSystemConfig(sys);
		...
		SystemConfig sys;
		config->getSystemConfig(sys);
//		sys.dataCollectionConsent = server.hasArg(F("sf")) && (server.arg(F("sf")) == F("true") || server.arg(F("sf")) == F("1")) ? 1 : 2;//EHorvat disabled consent stuff
		config->setSystemConfig(sys);
	}
	...
	if(server.hasArg(F("u")) && server.arg(F("u")) == F("true")) {
		...
		ui.showHourPlot = server.arg(F("uh")).toInt();  //EHorvat new hour plot
...
/*		config->setUpgradeInformation(0xFF, 0xFF, FirmwareVersion::VersionString, "");
		if(server.hasArg(F("url"))) {						//EHorvat disabled consent stuff
