...
void AmsDataStorage::update5Minute(int16_t wattimpexp) {
    for(int i = 119; i>0; i--) {
        Min15.mWatt[i] = Min15.mWatt[i-1] ;   
    }
    Min15.mWatt[0] = wattimpexp;
}

int32_t AmsDataStorage::get5MinuteWatt(uint8_t element) {
    if(element < 0 || element > 119) return 0;
    return Min15.mWatt[element];
}
