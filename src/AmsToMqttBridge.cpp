...
unsigned long Last_5second_millis = 0;
unsigned long Last_hourplot_avg = 0;
//EHorvat new hour plot new end
unsigned long Last_APClient_millis =millis(); //Used to auto AP swapp
...
//			if (hour_plot_avg > 0) (hour_plot_avg = -2230);//EHorvat for testing only
		    ...
		    index_60minplot = 0;
			summ_60minplot = 0;
		    if(Debug.isActive(RemoteDebug::INFO)) debugI("+++++ now millis: %i  +++ Hour Plot updateHour was done with: %d Watt\r\n", now, hour_plot_avg);
		}
	}
//EHorvat new hour plot new end
//EHorvat auto Wifi  swap swapWifiMode() start
...
//EHorvat auto Wifi  swap swapWifiMode() end
...
void swapWifiMode() {
	...
	uint16_t chipId;					// EHorvat new
	#if defined(ESP32)					// EHorvat new
		chipId = ESP.getEfuseMac();		// EHorvat new
	#else								// EHorvat new
		chipId = ESP.getChipId();		// EHorvat new
	#endif								// EHorvat new
...
//		WiFi.softAP(PSTR("AMS2MQTT"));// EHorvat disabled line
		...
		WiFi.softAP((String("amsreader-") + String(chipId, HEX)).c_str(), WIFI_AP_PASSWORD);  //EHorvat ... example to include AP password
//		WiFi.softAP((String("amsreader-") + String(chipId, HEX)).c_str());  // EHorvat ... no AP security
		WiFi.mode(WIFI_AP);
...
//		#if defined(DEBUG_MODE)   	//EHorvat Disabled...always debug now... also in AP mode
			Debug.setSerialEnabled(true);
			Debug.begin(F("192.168.4.1"), 23, RemoteDebug::VERBOSE);
//		#endif						//EHorvat Disabled...always debug now... also in AP mode
	} else {
...
//				ESP.restart();   //EHorvat disabled line
