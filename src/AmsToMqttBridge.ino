...
long array_60minplot[6] = {0,0,0,0,0,0};  	//EHorvat Array to build average of 6 samples for 60 minute plot
long avg_60minplot = 0;						//EHorvat
byte index_60minplot = 0;					//EHorvat
unsigned long last60MinPlotupdate = 0; 		//Ehorvat 
...
//	if (Debug.isActive(RemoteDebug::INFO)) {   //EHorvat always send this info
		debugI("AMS bridge started");
		debugI("Voltage: %.2fV", vcc);
		debugI("AP Pin: %d", gpioConfig.apPin);
//	}	
	float vccBootLimit = gpioConfig.vccBootLimit == 0 ? 0 : gpioConfig.vccBootLimit / 10.0;
/* EHorvat disabled Boot Limit
...
*/ //EHorvat disabled Boot Limit end
		...
		if (WiFi.status() != WL_CONNECTED) {
			...
			if(!wifiConnected) {
				...
				if(config.getWiFiConfig(wifi)) {
					...
					Serial.printf("Successfully connected to WiFi! \n");				//EHorvat ... always show this info
					Serial.printf("IP:  %s \n", WiFi.localIP().toString().c_str());	//EHorvat ... always show this info
					Serial.printf("AP Pin: %d \n", gpioConfig.apPin);	//EHorvat ... always show this info
					if(Debug.isActive(RemoteDebug::INFO)) {
...
//		}
	...
	if(now > 30000 && now - last60MinPlotupdate > 5000) {
		last60MinPlotupdate = now;
		// Prepare data for 60 Minute Plot (Sum up and Average 4 measurements .... 120 data points for 3600 sec --> average 6 samples of 5 sec interval)
		array_60minplot[index_60minplot] = (meterState.getActiveImportPower() - meterState.getActiveExportPower());
		avg_60minplot = 0;
		for (unsigned long i = 0; i < 6; i++) {
			avg_60minplot = avg_60minplot + array_60minplot[i];
		}
		if (index_60minplot >= 6) {
			ds.update5Minute(avg_60minplot / 6); //EHorvat put data to new 60 Minute Plot array
			index_60minplot = 0;
		} else { index_60minplot++;
		}
		//	ds.update5Minute(meterState.getActiveImportPower() - meterState.getActiveExportPower()); //EHorvat new for 5 Minute plot send Import - Export power
	}
	#if defined(ESP32)
		esp_task_wdt_reset();
	#elif defined(ESP8266)
...
bool readHanPort() {
//	debugV("**** Read HAN Port 1");  //EHorvat
	if(!hanSerial->available()) return false;
//	debugV("**** Read HAN Port 2"); //EHorvat
	...
	if(!serialInit) {
		...
		serialInit = true;
//		debugV("****Received Data: %s", hanBuffer); //EHorvat
		return false;
	}
...
void WiFi_connect() {
	...
	if (WiFi.status() != WL_CONNECTED) {
		if(WiFi.getMode() != WIFI_OFF) {
			if(wifiReconnectCount > 4320) {  		// EHorvat 4320 --> restart 3days interval (original number was 3)
				ESP.restart();
				return;
			}
...
//				debugD("MQTT SSL setup complete (%dkb free heap)", ESP.getFreeHeap());				
				debugD("MQTT SSL is disabled in this Version (%dkb free heap)", ESP.getFreeHeap());
//			}
//		}
