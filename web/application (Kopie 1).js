...
var drawDay = function() {
    $.ajax({
        ...
        data = [['Hour', 'Export', { role: 'style' }, { role: 'annotation' }, 'Import', { role: 'style' }, { role: 'annotation' }]];  //EHorvat ... changed: Import is negative.
        var r = 1;
        var hour = moment.utc().hours();
        var offset = moment().utcOffset()/60;
        var min = 0;
        for(var i = hour; i<24; i++) {
            ...
            data[r++] = [zeropad((i+offset)%24), exp, "opacity: 0.9;", exp == 0 ? "" : exp.toFixed(2), imp == 0 ? 0 : -imp, "opacity: 0.9;", imp == 0 ? "" : -imp.toFixed(2)];  //EHorvat ... changed: Import is negative.
...
var drawMonth = function() {
    $.ajax({
        ...
        for(var i = day; i<=eom; i++) {
            ...
            data[r++] = [zeropad(i), exp, "opacity: 0.9;", exp == 0 ? "" : exp.toFixed(1), imp == 0 ? 0 : -imp, "opacity: 0.9;", imp == 0 ? "" : -imp.toFixed(1)];    //EHorvat ... changed: Import is negative.
            //min = Math.min(0, -exp);   //EHorvat ... original: Import is positive
            min = Math.min(0, -imp); //EHorvat ... modified: Import is negative
        }
        for(var i = 1; i < day; i++) {
