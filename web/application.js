...
// //EHorvat 5 Minute plot (short plot)
var sp;
var sa;
var so = {
    title: 'letzte Stunde',  // ****** EHorvat was Energy use last 5 Minutes....... now last 10 minutes
    titleTextStyle: {
        fontSize: 14
    },
    //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
    colors: ['#6f42c1', '#28a745'],     //EHorvat ... original: Import is positive. Import is purple Export is green
    //colors: ['#28a745', '#6f42c1'],   //EHorvat ... changed: Import is negative. Import is purple Export is green
    backgroundColor: { fill:'#transparent' },
    legend: { position: 'none' },
    vAxis: {
        title: 'Watt',
...
var ea;
var eo = {
    title: 'letzte 24 Stunden',  //EHorvat was Energy use last 24 hours
    titleTextStyle: {
        fontSize: 14
    },
    //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
...
var ma;
var mo = {
    title: 'letzte 30 Tage', //EHorvat was  Energy use last month
    titleTextStyle: {
        fontSize: 14
    },
    //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
...
var va;
var vo = {
    title: 'Spannung',   //EHorvat was: Phase voltage
    titleTextStyle: {
        fontSize: 14
    },
    backgroundColor: { fill:'transparent' },
...
var aa;
var ao = {
    title: 'Strom',   //EHorvat was: Phase current
    titleTextStyle: {
        fontSize: 14
    },
    backgroundColor: { fill:'transparent' },
...
var xo = {
    ...
    pieSliceBorderColor: 'transparent',
    slices: {
// Show Export plot in green color --> set color to '#28a745' (was purple = '#6f42c1' )
        0: { color: '#28a745' },  //EHorvat was: green
        1: { color: '#eee' },
        2: { color: 'transparent' }
    },
    legend: { position: 'none' },
...
var setupChart = function() {
    ...
    sp = new google.visualization.LineChart(document.getElementById('sp'));  //EHorvat new 5 Minute plot  
    ...
    fetch();
    drawMinute();   //EHorvat new 5 Minute plot  
    drawDay();
    drawMonth();
};
...
var redraw = function() {
    ...
    sp.draw(sa, so);    //EHorvat new 5 Minute plot  
    ep.draw(ea, eo);
    mp.draw(ma, mo);
    vp.draw(va, vo);
    ap.draw(aa, ao);
...
var drawMinute = function() {
    $.ajax({
        ...
        //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
        data = [['Hour', 'Watt', { role: 'style' }]]; //  EHorvat ... original: Import is positive
        //data = [['Hour', 'Export', { role: 'style' }, { role: 'annotation' }, 'Import', { role: 'style' }, { role: 'annotation' }]];  //EHorvat exchanged Import / Export
        var r = 1;
        for(var i = 119; i > -1; i--) {   // Show current data on right side
            var val = json["i"+zeropad(i)];
            //   EHorvat .... next 4 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
            data[r++] = ["vor " + (Math.round(((i+1) *0.5) * 2.0)) / 2.0 + " Min.", val, "opacity: 0.9;"]; //  EHorvat ... just one line
            //data[r++] = [zeropad(i), exp, "opacity: 0.9;", exp == 0 ? "" : exp.toFixed(0), imp == 0 ? 0 : -imp, "opacity: 0.9;", imp == 0 ? "" : -imp.toFixed(0)]; //  EHorvat ... modified: Import is negative
        }
        sa = google.visualization.arrayToDataTable(data);
        sp.draw(sa, so);
        setTimeout(drawMinute,(2* interval));  //Timeout to 10 Sec
    });
};
...
var drawDay = function() {
    $.ajax({
        ...
        //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
        //data = [['Hour', 'Import', { role: 'style' }, { role: 'annotation' }, 'Export', { role: 'style' }, { role: 'annotation' }]];    //EHorvat ... original: Import is positive.
        data = [['Hour', 'Export', { role: 'style' }, 'Bezug', { role: 'style' }]];  //EHorvat ... changed: Import is negative.
        var r = 1;
        var hour = moment.utc().hours();
        var offset = moment().utcOffset()/60;
        var min = 0;
        for(var i = hour; i<24; i++) {
            ...
            //   EHorvat .... next 4 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
            //data[r++] = [zeropad((i+offset)%24), imp, "opacity: 0.9;", imp == 0 ? "" : imp.toFixed(1), exp == 0 ? 0 : -exp, "opacity: 0.9;", exp == 0 ? "" : -exp.toFixed(1)];    //EHorvat ... original: Import is positive.
            data[r++] = [zeropad((i+offset)%24), exp, "opacity: 0.9;", imp == 0 ? 0 : -imp, "opacity: 0.9;"];  //EHorvat ... changed: Import is negative.
            //min = Math.min(0, -exp);   //EHorvat ... original: Import is positive
            min = Math.min(0, -imp); //EHorvat ... modified: Import is negative
        };
...
var drawMonth = function() {
    $.ajax({
        ...
        //   EHorvat .... next 2 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
        //data = [['Hour', 'Import', { role: 'style' }, { role: 'annotation' }, 'Export', { role: 'style' }, { role: 'annotation' }]];    //EHorvat ... original: Import is positive.
        data = [['Hour', 'Export', { role: 'style' }, 'Import', { role: 'style' }]];  //EHorvat ... changed: Import is negative.
        var r = 1;
        var day = moment().date();
        ...
        for(var i = day; i<=eom; i++) {
            ...
            var exp = json["e"+zeropad(i)];
            //   EHorvat .... next 4 Lines....option to exchange "Export" with "Import" (to show Import as "negativ" values on plot)
            //data[r++] = [zeropad(i), imp, "opacity: 0.9;", imp == 0 ? "" : imp.toFixed(1), exp == 0 ? 0 : -exp, "opacity: 0.9;", exp == 0 ? "" : -exp.toFixed(1)];      //EHorvat ... original: Import is positive.
            data[r++] = [zeropad(i), exp, "opacity: 0.9;", imp == 0 ? 0 : -imp, "opacity: 0.9;"];    //EHorvat ... changed: Import is negative.
            //min = Math.min(0, -exp);   //EHorvat ... original: Import is positive
            min = Math.min(0, -imp); //EHorvat ... modified: Import is negative
        }
        for(var i = 1; i < day; i++) {
...
var voltcol = function(pct) {
    ...
    else if(pct > 35) return '#28a745'; //EHorvat was 32d900 
...
var ampcol = function(pct) {
    ...
    else return '#28a745'; //EHorvat was 32d900
};
    ...
    $.ajax({
        ...
        for(var id in json) {
            ...
            if(isNaN(str)) {
                ...
                $('.j'+id).html(num.toFixed(num < 0 ? 0 : num < 10 ? 3  : 2));   //EHorvat was: num < 10 ? 2  : 1));
            }
            $('.r'+id).show();
        }
        ...
        if(ip) {
            ...
            io.slices[0].color = '#6f42c1';  // EHorvat always show same color (Import = purple '#6f42c1' , Export = green '#28a745') was : = ampcol(pct)
        ...
        if(om > 0) {
            ...
            if(xp) {
                ...
                xo.slices[0].color = '#28a745';  // EHorvat always show same color (Import = purple '#6f42c1' , Export = green '#28a745') was : = ampcol(pct)
        ...
        if(vp) {
            switch(ds) {
                case 1:
                    ...
                    break;
                case 2:
                    vo.title = 'Spannung'; //EHorvat was: Phase voltage
                    break;
            }
            var c = 0;
            var t = 0;
        ...
        if(ap && json.mf) {
            switch(ds) {
                case 1:
                    ...
                    break;
                case 2:
                    ao.title = 'Strom';   //EHorvat was:  Phase current
                    break;
            }
            var dA = false;
            var r = 1;
        ...
        if(Math.abs(unixtime-ts) < 300) {
            $('.jc').html(moment(ts * 1000).format('DD.MM.YYYY HH:mm:ss')); //EHorvat added YYYY and ss
            $('.jc').removeClass('text-danger');
        } else  {
            $('.jc').html(moment(ts * 1000).format('DD.MM.YYYY HH:mm:ss'));  //EHorvat added ss
